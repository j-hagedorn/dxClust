...
#Import/View data
Diagnoses <- read.delim(".Diagnosis/DDDiagnosis2.txt", dec=",")
#View(Diagnoses)
attach(Diagnoses)
...
##Cluster Analysis
#dist <- dist(ClusterData2, method='binary')
#items <- hclust(dist, method='complete')
#plot(items)

#rm(Diagnoses, DiagnosesLvl3, ClusterData)

##Creating new data set ClusterData that excludes diagnoses that made up their own branch on the
#cluster dendrogram above
#attach(New)
#detach(New)
#keepdiag <- c('SexGenIdentDis','TicDis','OthViralInfect','RetDetach','UTI','OthDisBladder','SpinaBifida','Nephritis','Thyrotoxicosis',
#                            'Migraine','OthDisStomach','MentalDisGen','SkullFaceFract','LateEffCerebroDis','SomatoDis','HodgkinsDis','MotorSkillDis',
#                            'OthBackProb','GastLacerSyn','HeartValveDis','FractNeckFem','Hemiplegia','OthIntracInj','OthNonTraumaJointDis','MenopausDis',
#                            'EsophDis','Obesity','DownsSyn','OthThyroidDis','OthCentNerveDis','DisPeripNerv','Inflammation','DisMinMeta','OthCircDis',
#                            'DiaphHernia','RheuArthritis','DiabOptha','HypertensionComp','DiabNeuro','EssenHyper','OthAsthma','DeficientAnemia',
#                            'RegEnterCol','Convulsions','OthGastroDis','Constipation','Hemorrhoids','CongenAnomSkull','MenstDis','OthDisVeins','OthLowRespDis',
#                            'PulmHeartDis','OthCongenAnomInteg','ConductionDis','DiabCirc','CondDizz','CoronaryAthero','Concussion','OthHeadache','CompSurg',
#                            'Thrombocytopenia','Dysphagia','OthMetabolic','DevelopDis','CommDis','OthNervSysSympDis','CodesMHDis','NonmaligBreastCon','UndescTesticle',
#                            'CongenHydro','Blindness','OthEarDis','ConductDis','OppDefiDis','OthDisInfancy','DepressDis','LearnDis','Epilepsy','OthParalysis',
#                            'ADDADHD','PervaDevelDis','Bipolar','IntellDis','Appendicitis','Endometriosis','Osteoarthritis','Aortic','ObsChronBronch',
#                            'FractLowLimb','Cellulitis','VaricoseVeins','GenitSymp','OthHeredNervSysCond','Allergic','OthAcquireDeform','AllOthCogAnom',
#                            'CongenHip','AdminSocAdmiss','ElimDis','OthFemGenDis','OthEyeDis','AcuteRenalFail','OthBactInfect')

#ClusterData2 <- New[keepdiag]
##Transposing ClusterData so that diagnosis variables are rows and id's are columns
#ClusterData3 <- t(ClusterData2)
#View(ClusterData3)

##Cluster Analysis
#dist2 <- dist(ClusterData3, method='binary')
#items2 <- hclust(dist2, method='complete')
#plot(items2)

##Includes diagnoses with n >= 10
#rm(ClusterData2,ClusterData3)

#keepdiag2 <- c('Constipation','GenitSymp','HeartValveDis','Migraine','ElimDis','OthCentNerveDis','OthEyeDis','AdminSocAdmiss','OthHeredNervSysCond',
#               'SexGenIdentDis','TicDis','OthDisInfancy','Convulsions','OthIntracInj','Dysphagia','OthNonTraumaJointDis','OthAcquireDeform','Allergic',
#               'ConductDis','OppDefiDis','OthMetabolic','EsophDis','OthAsthma','AllOthCogAnom','Blindness','DevelopDis','OthEarDis','EssenHyper','Obesity',
#               'OthThyroidDis','CommDis','DownsSyn','OthNervSysSympDis','Epilepsy','ADDADHD','OthParalysis','PervaDevelDis','LearnDis','DepressDis','Bipolar','IntellDis')

#ClusterData2 <- New[keepdiag2]
##Transposing ClusterData so that diagnosis variables are rows and id's are columns
#ClusterData3 <- t(ClusterData2)
#View(ClusterData3)
...
##Includes diagnoses with n >= 20
rm(ClusterData2,ClusterData3)

keepdiag2 <- c('OthNonTraumaJointDis','OthAcquireDeform','Allergic',
               'ConductDis','OppDefiDis','OthMetabolic','EsophDis','OthAsthma','AllOthCogAnom','Blindness','DevelopDis','OthEarDis','EssenHyper','Obesity',
               'OthThyroidDis','CommDis','DownsSyn','OthNervSysSympDis','Epilepsy','ADDADHD','OthParalysis','PervaDevelDis','LearnDis','DepressDis','Bipolar','IntellDis')
...
#write.table(dist2new, "T:/SQL Query Library/Reports/DD Population/Diagnosis/Tables/Distance Matrix", sep="\t")
hclust <- hclust(dist2, method='complete')
lab <- c('Other Non-Traumatic Joint Disorders','Other Acquired Deformities','Allergic Reactions','Conduct Disorders',
         'Oppositional Defiant Disorder','Other & Unspecified Metabolic; Nutritional; & Endocrine Disorders',
         'Esophageal Disorders','Other & Unspecified Asthma','All Other Congenital Anomalies','Blindness & Vision Defects',
         'Developmental Disabilities','Other Ear & Sense Organ Disorders','Essential Hypertension','Obesity',
         'Other Thyroid Disorder','Communication Disorders','Downs Syndrome','Other Nervous System Symptoms & Disorders',
         'Epilepsy','ADD & ADHD','Other Paralysis','Pervasive Developmental Disorders','Learning Disorders','Depressive Disorders','Bipolar Disorders','Intellectual Disabilities')
lab.col <- c("blue","orange","blue","green","green","orange","red","blue","purple","purple","orange","purple","blue","red",
             "grey","orange","grey","orange","red","green","red","red","green","green","red","red")
...
text(x=x, y=y[hclust$order]-(max(hclust$height)*hang), labels=lab[hclust$order], col=lab.col[hclust$order], srt=90, adj=c(1,0.5), xpd=NA, cex=0.5)
##Additional Exploratory Analysis
##Two-way Frequencies
table(ClusterData2$OthThyroidDis,ClusterData2$DownsSyn)
fisher.test(table(ClusterData2$OthThyroidDis,ClusterData2$DownsSyn))
assocstats(table(ClusterData2$OthThyroidDis,ClusterData2$DownsSyn))

table(ClusterData2$Blindness,ClusterData2$OthEarDis)
fisher.test(table(ClusterData2$Blindness,ClusterData2$OthEarDis))
assocstats(table(ClusterData2$Blindness,ClusterData2$OthEarDis))

table(ClusterData2$EsophDis,ClusterData2$Obesity)
fisher.test(table(ClusterData2$EsophDis,ClusterData2$Obesity))
assocstats(table(ClusterData2$EsophDis,ClusterData2$Obesity))

table(ClusterData2$Epilepsy,ClusterData2$OthParalysis)
fisher.test(table(ClusterData2$Epilepsy,ClusterData2$OthParalysis))
assocstats(table(ClusterData2$Epilepsy,ClusterData2$OthParalysis))

table(ClusterData2$Bipolar,ClusterData2$IntellDis)
fisher.test(table(ClusterData2$Bipolar,ClusterData2$IntellDis))
assocstats(table(ClusterData2$Bipolar,ClusterData2$IntellDis))

table(ClusterData2$LearnDis,ClusterData2$DepressDis)
fisher.test(table(ClusterData2$LearnDis,ClusterData2$DepressDis))
assocstats(table(ClusterData2$LearnDis,ClusterData2$DepressDis))

table(ClusterData2$OppDefiDis,ClusterData2$ADDADHD)
fisher.test(table(ClusterData2$OppDefiDis,ClusterData2$ADDADHD))
assocstats(table(ClusterData2$OppDefiDis,ClusterData2$ADDADHD))

table(ClusterData2$OthAsthma,ClusterData2$EssenHyper)
fisher.test(table(ClusterData2$OthAsthma,ClusterData2$EssenHyper))
assocstats(table(ClusterData2$OthAsthma,ClusterData2$EssenHyper))

table(ClusterData2$OthNonTraumaJointDis,ClusterData2$Allergic)
fisher.test(table(ClusterData2$OthNonTraumaJointDis,ClusterData2$Allergic))
assocstats(table(ClusterData2$OthNonTraumaJointDis,ClusterData2$Allergic))

table(ClusterData2$CommDis,ClusterData2$OthNervSysSympDis)
fisher.test(table(ClusterData2$CommDis,ClusterData2$OthNervSysSympDis))
assocstats(table(ClusterData2$CommDis,ClusterData2$OthNervSysSympDis))

table(ClusterData2$OthAcquireDeform,ClusterData2$OthMetabolic)
fisher.test(table(ClusterData2$OthAcquireDeform,ClusterData2$OthMetabolic))
assocstats(table(ClusterData2$OthAcquireDeform,ClusterData2$OthMetabolic))

#Three-way Frequencies
table1 <- xtabs(~ClusterData2$AllOthCogAnom+ClusterData2$Blindness+ClusterData2$OthEarDis)
write.table(table1, "T:/SQL Query Library/Reports/DD Population/Diagnosis/Tables/3-way Table One", sep="\t")

table2 <- xtabs(~ClusterData2$PervaDevelDis+ClusterData2$Bipolar+ClusterData2$IntellDis)
write.table(table2, "T:/SQL Query Library/Reports/DD Population/Diagnosis/Tables/3-way Table Two", sep="\t")

table3 <- xtabs(~ClusterData2$ConductDis+ClusterData2$OppDefiDis+ClusterData2$ADDADHD)
write.table(table3, "T:/SQL Query Library/Reports/DD Population/Diagnosis/Tables/3-way Table Three", sep="\t")

table4 <- xtabs(~ClusterData2$DevelopDis+ClusterData2$CommDis+ClusterData2$OthNervSysSympDis)
write.table(table4, "T:/SQL Query Library/Reports/DD Population/Diagnosis/Tables/3-way Table Four", sep="\t")

##Includes diagnoses with n >= 37
#rm(ClusterData2,ClusterData3)

#keepdiag2 <- c('Blindness','DevelopDis','OthEarDis','EssenHyper','Obesity',
#               'OthThyroidDis','CommDis','DownsSyn','OthNervSysSympDis','Epilepsy','ADDADHD','OthParalysis','PervaDevelDis','LearnDis','DepressDis','Bipolar','IntellDis')
